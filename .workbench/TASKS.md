...
| T-1789010001 | 待办 | 验证link-mp-openid修复效果 | 待办 | 09-10 22:15 | | S123已修复(commit abe851b): upsert_phone_openid_row的UPDATE撞 idx_phone_openids_phone_unionid 唯一约束(一机两号场景). 该接口请求集中在早7点-下午3点(存包高峰, 今日分布07时99/08时446/09时403/10-15时逐渐减少/17时后为0), 修复22:04上线后当晚无请求, 无法实测. 【验证方法】次日上午7点后执行: journalctl -u smart-locker --since today --no-pager | grep -c "idx_phone_openids_phone_unionid"  期望=0(修复前每天17-56次); 另看 grep -c "unionid已被同号码其他行占用" 说明修复正在跳过unionid更新. 【逻辑已用真实数据验证】13037520586(一机两号: 行A unionid oTGtk2SNe0h../行B unionid oTGtk2ai_QPw..), 修复前会UPDATE行B的unionid为行A值而撞约束报500, 修复后检测到被占用则不改unionid只更新openid/mp_openid |
