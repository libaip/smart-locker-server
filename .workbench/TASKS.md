...
| T-1789005002 | 待办 | rollback在autocommit连接失效(半截数据隐患) | 待办 | 09-10 13:00 | | database.py:178 self._conn.autocommit=True -> get_db()每条SQL独立提交, 异常分支conn.rollback()完全无效 -> 任何写接口中途报错都留半截数据(本次结算500即留下batch+logs无payments); 影响: 所有用rollback兜底的接口(结算/退款/提现/投诉等); 加固建议: 关键写操作显式关autocommit或用显式事务, 或将可能失败的校验(含FOR UPDATE)提前到所有INSERT之前; 需排查同类风险点 |
