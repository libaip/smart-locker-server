...
| T-1789030300 | 待办 | app.py把405转成500(5xx告警失真的根源)+nginx屏蔽WordPress探测 | 待办 | 09-11 07:50 | | 起因: 05:18告警的23个5xx其实是扫描器(103.168.67.253)打WordPress漏洞探测(POST /?rest_route=/batch/v1, POST /, POST /admin, 带5454字节攻击载荷, Referer伪造/wp-admin/), 正确响应应该是405却被记成500; 根因: app.py:341 的 @app.errorhandler(Exception) 无差别捕获, 把 Flask 的 HTTPException(405/404/400)统一转成500并打完整traceback(今天43次"未捕获异常: 405"); 影响: (1)扫描器一打就触发5xx告警, 告警失去意义(今天46个5xx里39个是扫描器) (2)真实的405被掩盖成500, 干扰排查; 修法: handler里判断 isinstance(e, HTTPException) 时返回其自身状态码(4xx只warning不打traceback), 只有真异常才返回500; 【配套】nginx 加 WordPress 探测路径屏蔽(/wp*, /wordpress*, /blog*, /xmlrpc.php, index.php, ?rest_route=) 直接 404/444, 从源头消除噪音; 注意: 改 app.py 需按 SOP(106改 -> deploy_check -> rsync -> 重启5001+5002), 会短暂重启, 建议低峰期做 |
