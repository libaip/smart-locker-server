...
所有值都是从 106 服务器代码里读出来的，不是编的（只读服务器，没动它）。
"""
...
def seed(reset=False):
    """写种子数据。reset=True 会先清空账号池/模板/配置项（本地演示用）"""
    if reset:
        with C._conn() as (conn, kind):
            for t in ('wx_accounts', 'wx_templates', 'wx_switch_log'):
                C._exec(conn, kind, 'DELETE FROM %s' % t)
            C._exec(conn, kind, 'DELETE FROM wx_config_items')
        C.clear_cache()
...
if __name__ == '__main__':
    import sqlite3
    import os
    db = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'wx_config_center.db')
    C.bind(lambda: sqlite3.connect(db))
    C.init_db()
    print('种子写入:', seed(reset=True))
    print('当前生效:', C.resolve_all())
