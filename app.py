...
# withdrawal approval scheduler
def withdrawal_scheduler():
    import time, random
    import datetime
    from config import DATABASE
    _retry_counter = 0
    while True:
        time.sleep(30)
        try:
            _process_manual_batch()
            _process_queue_batch()
            # 每5分钟重试失败的退款记录（status=4）
            _retry_counter += 1
            if _retry_counter >= 10:
                _retry_counter = 0
                _retry_failed_refunds()
        except Exception as ex:
            logger.error("[Scheduler] " + str(ex))

def _retry_failed_refunds():
    """重试失败的退款记录（status=4，如商户余额不足）"""
    ...
    try:
        ...
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        # 查找 status=4 的记录（退款失败待重试）
        c.execute("SELECT * FROM withdrawal_records WHERE status=4 ORDER BY id ASC LIMIT 5")
        failed = c.fetchall()
        if not failed:
            conn.close()
            return
        for w in failed:
            ...
            # 获取订单的 payment_channel_id
            c2 = sqlite3.connect(DATABASE)
            c2.row_factory = sqlite3.Row
            c2c = c2.cursor()
            c2c.execute("SELECT payment_channel_id FROM orders WHERE id=%s", (w['order_id'],))
            order_row = c2c.fetchone()
            c2.close()
            if not order_row:
                continue
            payment_channel_id = order_row['payment_channel_id']
            ok, rid, msg = do_real_refund(order_id=w['order_id'], amount=w['amount'], payment_channel_id=payment_channel_id)
            if ok:
                # 重试成功
                c.execute("UPDATE withdrawal_records SET status=2, error_msg=NULL WHERE id=%s", (w['id'],))
                c.execute("UPDATE orders SET status=4, refund_id=%s, refund_time=%s WHERE id=%s", (rid, datetime.datetime.now(), w['order_id']))
                logger.info("[Retry] Success: withdrawal_id=%s, order_id=%s" % (w['id'], w['order_id']))
            else:
                # 仍然失败，更新错误信息
                c.execute("UPDATE withdrawal_records SET error_msg=%s WHERE id=%s", (msg, w['id']))
                logger.warning("[Retry] Failed: withdrawal_id=%s, msg=%s" % (w['id'], msg))
        conn.commit()
        conn.close()
    except Exception as e:
...
def _approve_wd(c, w):
    ...
    if w["order_id"]:
        from helpers import do_real_refund
        ok, rid, msg = do_real_refund(order_id=w["order_id"], amount=w["amount"])
        if ok:
            c.execute("UPDATE withdrawal_records SET status=2, approver='system_auto', approve_time=NOW(), refund_id=%s WHERE id=%s", (rid, w["id"]))
            c.execute("UPDATE orders SET status=4, refund_id=%s, refund_time=%s WHERE id=%s", (rid, datetime.datetime.now(), w["order_id"]))
        elif msg and ("已全额退款" in msg or "订单已全额退款" in msg):
            c.execute("UPDATE withdrawal_records SET status=2, approver='system_auto', approve_time=NOW(), error_msg=%s WHERE id=%s", (msg[:200], w["id"]))
            c.execute("UPDATE orders SET status=4, refund_id='', refund_time=NOW() WHERE id=%s AND status!=4", (w["order_id"],))
        else:
            c.execute("UPDATE withdrawal_records SET status=3, approver='system_auto', approve_time=NOW(), error_msg=%s WHERE id=%s", (msg[:200] if msg else "退款失败", w["id"]))
    else:
        from helpers import do_balance_transfer
        ok, pn, msg = do_balance_transfer(w["user_phone"], w["amount"])
        if ok:
            c.execute("UPDATE withdrawal_records SET status=2, approver='system_auto', approve_time=NOW(), refund_id=%s WHERE id=%s", (pn, w["id"]))
        else:
            _reject_wd(c, w)

def _reject_wd(c, w):
    from helpers import return_to_balance
    return_to_balance(w["user_phone"], w["amount"], w["id"])
...
threading.Thread(target=withdrawal_scheduler, daemon=True).start()
