...
def wx_login():
    ...
    try:
        ...
        result = resp.json()
        logger.info("[wx_login] code2Session keys: " + str(list(result.keys())))
        if 'openid' in result:
            openid_val = result['openid']  # 小程序的mp_openid
            openid_from_h5 = data.get('openid', '')  # H5传来的公众号openid
            unionid_val = result.get("unionid", "")
            ...
            try:
                ...
                conn2.execute("UPDATE phone_openids SET openid = %s, mp_openid = %s, unionid = COALESCE(unionid, %s) WHERE mp_openid = %s", (openid_val, openid_val, unionid_val, openid_val))
                conn2.commit()
                conn2.close()
            except:
                pass
            ...
            try:
                ...
                # 2.5 找不到，用unionid查
                if not _row and unionid_val:
                    _cur.execute("SELECT phone FROM phone_openids WHERE unionid = %s LIMIT 1", (unionid_val,))
                    _row = _cur.fetchone()
                ...
                if _row and _row[0]:
                    ...
                    if openid_from_h5:
                        _cur.execute("UPDATE phone_openids SET mp_openid = %s, unionid = COALESCE(unionid, %s) WHERE openid = %s", (openid_val, unionid_val, openid_from_h5))
                        _conn.commit()
                        ...
                        if _mgr and _mgr[0]:
                            ...
                            if _mgid:
                                for _dup in _mgc.execute("SELECT id, balance, total_deposited, total_withdrawn FROM user_balances WHERE phone = %s AND id != %s", (_phone, _mgid[0])).fetchall():
                                    ...
                                    _mgc.execute("UPDATE user_balances SET mp_openid = NULL, openid = NULL, balance = 0 WHERE id = %s", (_dup[0],))
                                _mg.commit()
            ...
            _resp = {'openid': result['openid'], 'session_key': result.get('session_key', ''), 'unionid': result.get('unionid', '')}
            if _phone:
                _resp['phone'] = _phone
        ...
        try:
            ...
            if phone_number:
                ...
                try:
                    ...
                    _conn = get_db()
                    _cur = _conn.cursor()
                    if openid.startswith('oLhbm2'):
                        _cur.execute("SELECT mp_openid FROM phone_openids WHERE phone = %s AND mp_openid IS NOT NULL AND mp_openid != '' AND mp_openid NOT LIKE 'oLhbm2%%' LIMIT 1", (phone_number,))
                        _existing_mp = _cur.fetchone()
                        if _existing_mp:
                            openid = _existing_mp[0]
...
def get_user_balance():
    ...
    try:
        ...
        if not row and phone:
            ...
            row = cur.fetchone()
        # [FIX] SUM balances via phone_openids bridge
        if row:
            _p = row["phone"]
            cur.execute("SELECT DISTINCT phone FROM phone_openids WHERE mp_openid = %s AND phone IS NOT NULL AND phone != ''", (mp_openid,))
            _ap = [r[0] for r in cur.fetchall()]
            if _p not in _ap:
                _ap.append(_p)
            cur.execute("SELECT COALESCE(SUM(balance),0) as bal, COALESCE(SUM(total_deposited),0) as tdep, COALESCE(SUM(total_withdrawn),0) as twd FROM user_balances WHERE phone = ANY(%s)", (_ap,))
            _br = cur.fetchone()
            if _br:
                row["balance"] = float(_br["bal"])
                row["total_deposited"] = float(_br["tdep"])
                row["total_withdrawn"] = float(_br["twd"])
...
def user_withdraw():
    ...
    try:
        ...
        balance = row['balance']
        _all_p = [phone]  # 默认只查当前手机号，后面会补充
        try:
            cursor.execute("SELECT DISTINCT phone FROM phone_openids WHERE mp_openid = %s AND phone IS NOT NULL AND phone != ''", (mp_openid,))
            _all_p = [r[0] for r in cursor.fetchall()]
            if phone not in _all_p:
                _all_p.append(phone)
            cursor.execute("SELECT COALESCE(SUM(balance),0) FROM user_balances WHERE phone = ANY(%s)", (_all_p,))
        ...
        if withdraw_mode == 'auto_approve':
            ...
            WHERE bd.user_phone = ANY(%s) AND bd.status='available' AND o.transaction_id IS NOT NULL AND o.transaction_id != ''
            ORDER BY bd.id DESC"""
            cursor.execute(sql, (_all_p,))
            ...
            # 扣除余额
            cursor.execute('UPDATE user_balances SET balance = GREATEST(balance - %s, 0) WHERE phone = ANY(%s) AND balance > 0',
            ...
            cursor.execute("SELECT bd.id, bd.order_id, bd.amount, o.transaction_id, o.openid as order_openid FROM user_balance_details bd JOIN orders o ON bd.order_id = o.id WHERE bd.user_phone = ANY(%s) AND bd.status='available' AND o.transaction_id IS NOT NULL AND o.transaction_id != '' ORDER BY bd.id DESC", (_all_p,))
            balance_records = cursor.fetchall()
            if not balance_records:
                conn.close()
...
def link_openid():
    try:
        ...
        if existing:
            ...
            # 没找到，正常插入
            if mp_openid_val and mp_openid_val.startswith('oLhbm2'):
                cursor.execute("SELECT mp_openid FROM phone_openids WHERE phone = %s AND mp_openid IS NOT NULL AND mp_openid != '' AND mp_openid NOT LIKE 'oLhbm2%%' LIMIT 1", (phone,))
                _existing_mp = cursor.fetchone()
                if _existing_mp:
                    mp_openid_val = _existing_mp[0]
