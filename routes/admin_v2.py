...
def admin_dashboard():
    ...
    try:
        ...
        c.execute('''SELECT
    (SELECT COUNT(DISTINCT user_phone) FROM orders WHERE status IN (2,3,4)) as cnt,
    COALESCE((SELECT SUM(deposit_amount) FROM orders WHERE status IN (2,3)),0)
        - COALESCE((SELECT SUM(deposit_amount) FROM orders WHERE refund_status='refunded'),0)
        - COALESCE((SELECT SUM(amount) FROM withdrawal_records WHERE status=2),0)
        - COALESCE((SELECT SUM(amount) FROM withdrawal_records WHERE status=0),0) as bal,
    COALESCE((SELECT SUM(deposit_amount) FROM orders WHERE status IN (2,3)),0) as dep,
    COALESCE((SELECT SUM(amount) FROM withdrawal_records WHERE status=2),0) as wd
''')
        ...
        c.execute('SELECT COUNT(DISTINCT user_phone) FROM orders WHERE status IN (2,3,4)')
        ...
        for row in c.fetchall():
            ...
            if not d['is_online'] and d.get('last_heartbeat'):
                try:
                    ...
                    d['is_online'] = (now - hb).total_seconds() < 120
                except:
                    pass
    ...
    try:
        ...
        if device_id:
            ...
            if cab:
                ...
                conn3.commit()
                # Mark completed - prevent duplicate polling
                c3.execute("UPDATE pending_lock_cmds SET status='completed' WHERE status='pending' AND device_id=%s AND cabinet_id=%s", (device_id, cab['id']))
                conn3.commit()
...
def admin_slots_open_all():
    ...
    try:
        ...
        c.execute('SELECT slot_number, board_no, lock_no FROM cabinet_slots WHERE cabinet_id=%s AND status IN (1,2) ORDER BY board_no, lock_no', (cabinet_id,))
        slots2 = c.fetchall()
        for s in slots2:
            send_open_lock(str(row['mainboard_device_id']), s['board_no'], s['lock_no'], None, '', slot_number=s['slot_number'])
                ...
                for wr in pending_records:
                    try:
                        ...
                        if wr['order_id']:
                            ...
                            refund_success, refund_id, refund_msg, _ = do_real_refund(order_id=wr['order_id'], amount=wr['amount'], openid=wr.get('openid',''))
                        else:
                            # 余额提现，找最近订单原路退
                            ...
                            if recent:
                                from helpers import do_real_refund
                                refund_success, refund_id, refund_msg, _ = do_real_refund(order_id=recent['id'], amount=wr['amount'], openid=wr.get('openid',''))
...
def admin_order_refund():
    ...
    try:
        ...
        # 创建提现记录
        c.execute("INSERT INTO withdrawal_records (order_id, user_phone, amount, status, approver, created_at) VALUES (%s, %s, %s, 2, '管理员-自动退款', NOW())", (order_id, order_dict.get("user_phone", ""), amount))
        # 记录payments退款流水
        ...
        if order_dict.get('status') == 3 and order_dict.get('user_phone'):
            logger.info('[余额操作] 订单退款扣余额 phone=%s amount=%s', order_dict.get('user_phone'), amount)
...
def admin_member_refund():
    ...
    try:
        ...
        logger.info('[余额操作] 余额提现扣余额 order_dict.get(" user_phone)%s amount=%s', phone, refund_amount)
...
def admin_member_batch_refund():
    ...
    try:
        ...
        for phone in phones:
            ...
            logger.info('[余额操作] 清空余额 phone=%s', phone)
        ...
        conn.commit()
        conn.close()
        logger.info(f'[batch_refund] 批量退款: {success_count}人, 金额={total_refund:.2f}, admin={session.get("admin_username","")}')
...
def admin_withdrawal_approve():
    ...
    try:
        ...
        if order_ids_list and len(order_ids_list) > 0:
            ...
            for oid in order_ids_list:
                ...
                if od:
                    ...
                    if refund_this > 0.001:
                        ok, rid, rmsg, _ = do_real_refund(order_id=oid, amount=refund_this, payment_channel_id=wd.get('payment_channel_id'))
            ...
            conn.commit()
            conn.close()
            logger.info(f'[withdrawal_approve] 批量审批: id={withdrawal_id}, phone={phone}, amount={amount}, all_ok={all_ok}, admin={session.get("admin_username","")}')
            return json_response(message='审批通过，退款已处理' if all_ok else '审批通过，部分退款失败')
        # 兼容旧逻辑：单个order_id
        # 余额已在用户提现时扣除，无需再次扣除
        logger.info('[余额操作] 后台审批扣余额 order_dict.get(" user_phone)%s amount=%s', phone, amount)
        ...
        if order_id:
            ...
            if _refund_row and _refund_row[0] == 'refunded':
                ...
                # 订单押金退款
                from helpers import do_real_refund
                refund_success, refund_id, refund_msg, _ = do_real_refund(order_id=order_id, amount=amount, payment_channel_id=wd.get('payment_channel_id'))
        else:
        ...
        conn.close()
        if refund_success or (" 订单已全额退款" in str(refund_msg)):
            logger.info(f'[withdrawal_approve] 审批通过: id={withdrawal_id}, phone={phone}, amount={amount}, refund_id={refund_id}, admin={session.get("admin_username","")}')
            return json_response(message='审批通过，退款已完成')
        else:
            logger.info(f'[withdrawal_approve] 审批失败: id={withdrawal_id}, phone={phone}, amount={amount}, msg={refund_msg}, admin={session.get("admin_username","")}')
...
def admin_withdrawal_reject():
    ...
    try:
        ...
        if wd['status'] in (0, 1) and wd['user_phone']:
            logger.info('[余额操作] 退款失败退余额 order_dict.get(" user_phone)%s amount=%s', wd['user_phone'], wd['amount'])
        ...
        conn.commit()
        conn.close()
        logger.info(f'[withdrawal_reject] 拒绝: id={withdrawal_id}, phone={wd["user_phone"]}, amount={wd["amount"]}, reason={reason}, admin={session.get("admin_username","")}')
...
def admin_withdrawal_confirm_refund():
    ...
    try:
        ...
        conn.commit()
        conn.close()
        logger.info(f'[withdrawal_confirm_refund] 确认退款: id={withdrawal_id}, admin={session.get("admin_username","")}')
...
def admin_agent_save():
    try:
        ...
        c = conn.cursor()
        if data.get('id'):
            fields = ['name','contact_name','contact_phone','status','commission_rate','dashboard_config']
            sets, params = [], []
            for f in fields:
                if f in data:
                    sets.append(f'{f}=%s')
            ...
            c.execute('INSERT INTO agents (name, contact_name, contact_phone, password_hash, commission_rate, plain_password, dashboard_config) VALUES (%s,%s,%s,%s,%s,%s,%s)',
...
def admin_channel_save():
    try:
        ...
        c = conn.cursor()
        if data.get('id'):
            fields = ['name','channel_type','app_id','mch_id','api_key','app_secret','cert_name','cert_serial_no','is_active','rotation_index','is_sub_merchant','service_provider_mch_id','service_provider_app_id']
            sets, params = [], []
            for f in fields:
                if f in data:
                    sets.append(f'{f}=%s')
            ...
            import re
            if not re.match(r'^[a-zA-Z0-9_]+$', str(cert_name_file)):
                return json_response(code=400, message='Invalid cert name')
            if cert_name_file:
                import os
            ...
            c.execute('''INSERT INTO payment_channels (name,channel_type,app_id,mch_id,api_key,app_secret,cert_name,cert_serial_no,is_active,rotation_index,is_sub_merchant,service_provider_mch_id,service_provider_app_id) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)''',
                      (data.get('name'), data.get('channel_type'), data.get('app_id') or WX_APP_ID,
                       data.get('mch_id'), data.get('api_key') or WX_API_KEY, data.get('app_secret') or WX_APP_SECRET, data.get('cert_name'), cert_serial, data.get('status',1), data.get('rotation_index', 0),
                       data.get('is_sub_merchant',0), data.get('service_provider_mch_id',''), data.get('service_provider_app_id','')))
        conn.commit()
        conn.close()
        return json_response(message='保存成功')
    except Exception as e:
...
@bp.route('/admin/channel/api-key', methods=['POST'])
@require_auth
def admin_channel_api_key():
    try:
        data = request.get_json()
        mch_id = data.get('mch_id', '').strip()
        if not mch_id:
        ...
        conn = get_db()
        c = conn.cursor()
        c.execute("SELECT api_key, app_secret FROM payment_channels WHERE mch_id=%s AND api_key IS NOT NULL AND api_key != '' LIMIT 1", (mch_id,))
        row = c.fetchone()
        conn.close()
        if row:
            return json_response(data={'api_key': row['api_key'], 'app_secret': row.get('app_secret', '') or ''})
        return json_response(data={})
    except Exception as e:
        logger.error(f'[channel_api_key] {e}')
        return json_response(data={})
    ...
    try:
        ...
        if action == 'approve':
            ...
            if r and r['user_phone']:
                logger.info('[余额操作] 后台拒绝退余额 order_dict.get(" user_phone)%s amount=%s', r['user_phone'], r['amount'])
                c.execute("UPDATE user_balances SET balance=balance+%s,total_withdrawn=total_withdrawn-%s WHERE phone=%s",
...
def fund_flow_list():
    ...
    COALESCE(SUM(CASE WHEN o.status IN (2,3) THEN o.deposit_amount ELSE 0 END),0)
        ...
        - COALESCE((SELECT COALESCE(SUM(wr.amount),0) FROM withdrawal_records wr WHERE wr.status=2 AND wr.order_id = o.id),0) as balance,
    COALESCE(SUM(CASE WHEN o.status IN (2,3) THEN o.deposit_amount ELSE 0 END),0) as total_deposited,
    COALESCE((SELECT COALESCE(SUM(wr.amount),0) FROM withdrawal_records wr WHERE wr.status=2 AND wr.order_id = o.id),0) as total_withdrawn,
    MIN(o.created_at) as first_use_time,
    COUNT(o.id) as order_count
FROM orders o
GROUP BY o.user_phone
ORDER BY first_use_time DESC""")
        rows = [dict(r) for r in c.fetchall()]
        total_balance = sum(float(r.get('balance',0) or 0) for r in rows)
        total_users = len(rows)
        conn.close()
        return json_response(data={'list': rows, 'total_users': total_users, 'total_balance': total_balance})
    except Exception as e:
        ...
        for row in rows:
            if row.get('last_heartbeat'):
                try:
                    ...
                    if isinstance(hb, str):
                        hb = dt_mod.datetime.strptime(hb, '%Y-%m-%d %H:%M:%S')
                    diff = (dt_mod.datetime.now() - hb).total_seconds()
...
def cabinet_groups_by_code():
    return jsonify({'code': 200, 'data': {'cabinets': []}})
    try:
...
def merchant_login():
    try:
        ...
        import secrets
        token = secrets.token_hex(16)
        c.execute("UPDATE merchants SET auth_token=%s, login_attempts=0, is_locked=0, last_login_at=NOW() WHERE id=%s", (token, row['id']))
        ...
        for r in rows:
            try:
                ...
                if _rnd.random() < rate:
                    ...
                    amt = r['amount']
                    # ??????
                    lc2.execute('SELECT o.order_no, o.payment_channel_id FROM orders o WHERE id=%s', (order_id,))
                    ord = lc2.fetchone()
                    if ord:
                        success, refund_id, msg, _ = do_real_refund(order_id=order_id, order_no=ord['order_no'], amount=amt, payment_channel_id=ord['payment_channel_id'])
                        if success:
                            lc2.execute("UPDATE withdrawal_records SET status=2, approve_time=NOW(), approver='自动' WHERE id=%s", (r['id'],))
                            lc2.execute("UPDATE orders SET status=4, refund_id=%s, refund_time=NOW() WHERE id=%s", (refund_id, order_id))
                            approved += 1
                        else:
                            lc2.execute("UPDATE withdrawal_records SET status=1, approve_time=NOW(), approver='自动' WHERE id=%s", (r['id'],))
                else:
                    # ?????????????
                    logger.info('[余额操作] 队列拒绝退余额 order_dict.get(" user_phone)%s amount=%s', r['user_phone'], r['amount'])
                    lc.execute("UPDATE user_balances SET balance = balance + %s, total_withdrawn = total_withdrawn - %s WHERE phone = %s",
                              (r['amount'], r['amount'], r['user_phone']))
                    if lc.rowcount == 0:
                        lc.execute("INSERT INTO user_balances (phone, balance, total_withdrawn, first_use_time) VALUES (%s, %s, 0, NOW())",
                                  (r['user_phone'], r['amount']))
                    # 恢复余额明细状态
                    try:
                        _oids_str = r.get('order_ids') or '[]'
                        if _oids_str and _oids_str != '[]':
                            _oids = json.loads(_oids_str)
                        else:
                            _oids = [str(r['order_id'])] if r.get('order_id') else []
                        if _oids:
                        ...
                        pass
                    logger.info('[队列审批拒绝] id=%s order_dict.get(" user_phone)%s amount=%s order_ids=%s', r['id'], r['user_phone'], r['amount'], r.get('order_ids', '[]'))
                    logger.info('[队列审批拒绝] id=%s order_dict.get(" user_phone)%s amount=%s order_ids=%s', r['id'], r['user_phone'], r['amount'], r.get('order_ids', '[]'))
                    lc.execute("UPDATE withdrawal_records SET status=3, approve_time=NOW(), approver='队列' WHERE id=%s", (r['id'],))
                    rejected += 1
                local_conn.commit()
                local_conn.close()
            except Exception as _e:
                logger.error('[batch_auto] \u5904\u7406\u63d0\u73b0\u5931\u8d25 id=%s: %s', r['id'], _e)
        conn.close()
        return jsonify({'code': 200, 'message': f'队列审批完成: 通过{approved}笔, 拒绝{rejected}笔', 'data': {'approved': approved, 'rejected': rejected}})
    except Exception as e:
        return jsonify({'code': 500, 'message': str(e)})



@bp.route('/admin/withdrawal/batch-manual', methods=['POST'])
def withdrawal_batch_manual():
    """人工审批定时执行 - 到期自动处理（通过率命中后直接退款）"""
    try:
        from database import get_db
        import random as _rnd
        conn = get_db()
        ...
        for r in rows:
            try:
                ...
                if _rnd.random() < rate:
                    ...
                    order_id = r['order_id']
                    amt = r['amount']
                    lc2.execute('SELECT order_no, payment_channel_id FROM orders WHERE id=%s', (order_id,))
                    ord = lc2.fetchone()
                    if ord:
                        success, refund_id, msg, _ = do_real_refund(order_id=order_id, order_no=ord['order_no'], amount=amt, payment_channel_id=ord['payment_channel_id'])
                        if success:
                    ...
                    if _oid:
                        logger.info('[余额操作] 人工拒绝退余额 order_dict.get(" user_phone)%s amount=%s', r['user_phone'], r['amount'])
                        lc.execute("UPDATE user_balances SET balance=balance+%s, total_withdrawn=total_withdrawn-%s WHERE openid=%s", (r['amount'], r['amount'], _oid))
                    if _oid == '' or lc.rowcount == 0:
                        logger.info('[余额操作] 人工拒绝退余额2 order_dict.get(" user_phone)%s amount=%s', r['user_phone'], r['amount'])
                        lc.execute("UPDATE user_balances SET balance=balance+%s, total_withdrawn=total_withdrawn-%s WHERE phone=%s", (r['amount'], r['amount'], r['user_phone']))
                        if lc.rowcount == 0:
                            lc.execute("INSERT INTO user_balances (phone, balance, total_withdrawn, first_use_time) VALUES (%s, %s, 0, NOW())",
                                      (r['user_phone'], r['amount']))
                    # 恢复余额明细状态
                    try:
                        ...
                        pass
                    logger.info('[人工审批拒绝] id=%s order_dict.get(" user_phone)%s amount=%s order_ids=%s', r['id'], r['user_phone'], r['amount'], r.get('order_ids', '[]'))
                    logger.info('[人工审批拒绝] id=%s order_dict.get(" user_phone)%s amount=%s order_ids=%s', r['id'], r['user_phone'], r['amount'], r.get('order_ids', '[]'))
                ...
                logger.error('[batch_manual] \u5904\u7406\u63d0\u73b0\u5931\u8d25 id=%s: %s', r['id'], _e)
...
def admin_historical_orders_setting():
    try:
        conn = get_db()
        c = conn.cursor()
