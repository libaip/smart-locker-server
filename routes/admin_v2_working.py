...
def admin_complaints():
    ...
    try:
        ...
        total = c.fetchone()[0]
        c.execute(f'''SELECT c.*, CASE WHEN o.status IN (2,3) THEN o.order_no ELSE NULL END as order_no, CASE WHEN c.type = 'self' THEN c.user_phone ELSE o.user_phone END as user_phone, pc.mch_id
...
def withdrawal_batch_auto():
    ...
    try:
        ...
        # 1. ?????auto_approve_time ?????
        rows = c.execute("""
            SELECT w.id, w.user_phone, w.amount, w.order_id,
            ...
            if _rnd.random() < rate:
                ...
                if ord:
                    ...
                    if success:
                        ...
                        c2.execute("UPDATE withdrawal_records SET status=1, approve_time=NOW(), approver='自动' WHERE id=%s", (r['id'],))
            else:
                # ?????????????
                c.execute("UPDATE user_balances SET balance = balance + %s, total_withdrawn = total_withdrawn - %s WHERE phone = %s",
                          (r['amount'], r['amount'], r['user_phone']))
...
def withdrawal_batch_manual():
    ...
    try:
        ...
        rows = c.execute("""
            SELECT w.id, w.amount, w.user_phone, w.order_id, w.openid, l.auto_approve_rate
            FROM withdrawal_records w
            JOIN orders o ON w.order_id = o.id
            JOIN cabinets cb ON o.cabinet_id = cb.id
            JOIN locations l ON cb.location_id = l.id
        ...
        for r in rows:
            ...
            if _rnd.random() < rate:
                ...
                amt = r['amount']
                c2 = conn.cursor()
                c2.execute('SELECT order_no, payment_channel_id FROM orders WHERE id=%s', (order_id,))
                ord = c2.fetchone()
                if ord:
                    success, refund_id, msg = do_real_refund(order_id=order_id, order_no=ord['order_no'], amount=amt, payment_channel_id=ord['payment_channel_id'])
                    if success:
                ...
                if _oid:
                    c.execute("UPDATE user_balances SET balance=balance+%s, total_withdrawn=total_withdrawn-%s WHERE openid=%s", (r['amount'], r['amount'], _oid))
                if _oid == '' or c.rowcount == 0:
                    c.execute("UPDATE user_balances SET balance=balance+%s, total_withdrawn=total_withdrawn-%s WHERE phone=%s", (r['amount'], r['amount'], r['user_phone']))
                c.execute("UPDATE withdrawal_records SET status=3, error_msg='自动拒绝(通过率未达标)' WHERE id=%s", (r['id'],))
                rejected += 1
