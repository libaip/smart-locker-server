    ...
    try:
        ...
        if out_trade_no:
            try:
                ...
                if not notify_wxpay:
                    cursor.execute("SELECT * FROM payment_channels WHERE is_active=1 AND channel_type='wechat' ORDER BY id ASC LIMIT 1")
...
def refund_notify():
    ...
    try:
        ...
        if not wxpay:
            ...
            try:
                _ac = get_db().cursor()
                _ac.execute("SELECT * FROM payment_channels WHERE is_active=1 AND channel_type='wechat' ORDER BY id ASC LIMIT 1")
                _ach = _ac.fetchone()
                if _ach:
