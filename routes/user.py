...
def get_user_withdrawals():
    ...
    try:
        ...
        if withdraw_mode == 'auto_approve':
            ...
            if not balance_records:
                conn.close()
                return json_response(message='no refundable orders', code=400)
            total_refundable = 0
            order_refund_plan = []
            for br in balance_records:
                refundable = float(br['amount'])
            ...
            if total_refundable <= 0:
                conn.close()
                return json_response(message='no refundable amount', code=400)
            actual_amount = min(float(amount), total_refundable)
            if openid:
                cursor.execute('UPDATE user_balances SET balance = balance - %s, total_withdrawn = total_withdrawn + %s WHERE phone = %s AND openid = %s',
                               (actual_amount, actual_amount, phone, openid))
            ...
            conn.close()
            if all_ok:
                return json_response(data={'withdrawal_id': first_wid, 'status': 'refunded', 'amount': actual_amount, 'message': 'withdrawal successful'})
            else:
                return json_response(data={'withdrawal_id': first_wid, 'status': 'partial_refund', 'amount': actual_amount, 'message': 'partial refund processed'})
import psycopg2
from psycopg2.extras import RealDictCursor
"""
用户端API - Blueprint
    ...
    try:
        ...
        try:
            ...
            if device_id and board_no and lock_no:
                ...
                conn2 = get_db()
                c2 = conn2.cursor()
                c2.execute("UPDATE orders SET status=3, retrieve_time=NOW(), refund_mark=1, refund_amount=%s WHERE id=%s AND status=2",
                          (order_dict['deposit_amount'], order_dict['id']))
                if order_dict.get('slot_id'):
                    c2.execute("UPDATE cabinet_slots SET status=1 WHERE id=%s", (order_dict['slot_id'],))
                c2.execute("INSERT INTO user_balance_details (user_phone, order_id, amount, status) VALUES (%s,%s,%s,'available') ON CONFLICT (order_id) DO NOTHING",
