...
def _record_alert(device_id, alert_type, message):
    ...
    try:
        # ensure device_alerts table exists on startup
        try:
            _tmp_s3 = __import__('sqlite3')
            _tmp_db = _tmp_s3.connect('/home/ubuntu/smart-locker/locker.db', timeout=30)
            _tmp_cur = _tmp_db.cursor()
            _tmp_cur.execute('''CREATE TABLE IF NOT EXISTS device_alerts (
            ...
            if hb:
                try:
                    if isinstance(hb, str):
                        ...
                        hb_time = hb
                    device_online_status[dev_id] = False  # always start as offline, let WS trigger online alert
                except:
                                ...
                                if did:
                                    ...
                                    try:
                                        ...
                                        if reg_ver:
                                            ...
                                            try:
                                                _ac = db.cursor()
                                                _ac.execute("SELECT version_name, version_code FROM apk_version ORDER BY id DESC LIMIT 1")
                                                _ar = _ac.fetchone()
                                                if _ar:
                                                    db.execute("UPDATE cabinets SET last_heartbeat=NOW(), app_version=%s, app_version_code=%s WHERE mainboard_device_id=%s", (_ar[0], _ar[1] or 0, device_id))
                                                else:
                                ...
                                try:
                                    ...
                                    if version:
                                        ...
                                        db.execute(
                                            "UPDATE cabinets SET last_heartbeat=NOW() WHERE mainboard_device_id=%s", (device_id,))
                                    db.commit()
                                    # check if app_version is NULL
                                    try:
                                        _cv = db.cursor()
                                        _cv.execute("SELECT app_version FROM cabinets WHERE mainboard_device_id=%s", (device_id,))
                                        _crow = _cv.fetchone()
                                        if _crow and (not _crow[0] or _crow[0] == ""):
                                            import time
                                            _now_ts = time.time()
                                            _last_q = _version_query_pending.get(device_id, 0)
                                            if _now_ts - _last_q > 60:
                                                _version_query_pending[device_id] = _now_ts
                                                logger.info("[WS-MW] device " + str(device_id) + " version null, force reconnect")
                                                _record_alert(device_id, "offline", "forced reconnect for version")
                                                ws.close()
                                                break
                                    except Exception as __qv:
                                        logger.error("[WS-MW] version query error: " + str(__qv))
                                    db.close()
                                    # 版本更新改为推送下载模式，通过 /api/admin/device/push-update 手动触发
                                except:
                                    pass
